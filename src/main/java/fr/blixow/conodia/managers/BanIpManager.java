...
public class BanIpManager {
  ...
  private BanIpManager(String targetIp, String targetName, String authorName, String reason, long endTime, long startTime, boolean isPermanent, boolean isActive, int id) {
  ...
  public BanIpManager(String targetIp, String targetName, String authorName, String reason, long endTime, long startTime, boolean isPermanent) {
    this.targetIp = targetIp;
    this.targetName = targetName;
    this.authorName = authorName;
    this.reason = reason;
  ...
  public void handle() {
    ...
    CacheManager.getBanIpCache().add(this);

    this.id = Integer.parseInt(payload.get("id").toString());
  }

  public void undo() {
    ConodiaGameAPI.getInstance().getApiManager().destroy("/sanctions/" + id + "/ip_ban", new JSONObject());
    CacheManager.getBanIpCache().remove(this);
  }

  private JSONObject toJson() {
    JSONObject json = new JSONObject();
    json.put("type", "ip_ban");
    json.put("playerName", targetName);
    json.put("authorName", authorName);
    json.put("ip", targetIp);
  ...
  static BanIpManager from(JSONObject payload) {
    return new BanIpManager(
